...
sudo apt install pdsh -y > /dev/null 2>&1
echo "Done..."

echo "Running Script on compute node $NODE_IP with server IP $SERVER_IP..."
ssh $NODE_IP "bash -s" < ~/scripts/node/installChrony.sh "$SERVER_IP"
